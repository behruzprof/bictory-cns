...
export const HomePageSelectors = {
  domains: createSelector(
    [HomePageDomains.domains, GlobalDomains.favoriteDomains],
    (domains, favoriteList) => {
      /*  if (favoriteList?.length) {
        const mergedDomains = domains?.map((domain) => {
          favoriteList.find((domainName) => {
            if (domainName === domain.name) domain.favorite = true
          })
          return domain
        })
