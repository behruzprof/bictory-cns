...
function* addNewCardBySearchedDomain() {
  const foundDomain = yield select(HomePageSelectors.searchedDomain)
  const favoriteDomains = yield select(GlobalSelectors.favoriteDomains)
  let favorite = false
  if (favoriteDomains && favoriteDomains.length > 0) {
    favorite = favoriteDomains.some(
      (favDomain: string) => favDomain === foundDomain.name
    )
  }
  if (foundDomain.available) {
    let status = StatusEnum.available
    if (foundDomain.price.primary.value === 0) {
    ...
    yield put(HomePageActions.setDomains([newDomain]))
  } else {
    const newDomain = {
      id: foundDomain.entity.id,
      name: foundDomain.name,
      status: foundDomain.entity.status,
      favorite,
      auction: foundDomain.entity.auction,
    }
